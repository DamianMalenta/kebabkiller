...
export default function Studio() {
  ...
  useEffect(() => {
    Promise.all([api.characters.list(), api.backgrounds.list()])
      .then(([chars, bgs]) => {
        setCharacters(chars);
        setBackgrounds(bgs);
    ...
    <div className="space-y-8">
      ...
      {lastJob && (
        <div className="rounded-xl border border-emerald-900 bg-emerald-950/30 p-4">
          ...
          <p className="mt-1 text-sm text-zinc-400">ID: {lastJob.id} · Status: {lastJob.status}</p>
          <p className="mt-1 text-xs text-zinc-500">Śledź postęp na Dashboard.</p>
        </div>
      )}
    </div>
  );
